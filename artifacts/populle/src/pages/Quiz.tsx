...
import { getRandomQuestions, QuizQuestion, QuestionType } from '@/data/quizQuestions';
import {
  getPersonalityForScore,
  getPersonalityBySlug,
  buildQuizSharePayload,
} from '@/data/quizPersonalities';
import { Trophy, RefreshCw, ChevronRight, Target, Zap, Globe2, Building2, Clock, Flag } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { cn } from '@/lib/utils';

const TOTAL_ROUNDS = 10;
const MAX_SCORE = TOTAL_ROUNDS * 12;
...
const TYPE_COLORS: Record<QuestionType, string> = {
  ...
  comparison: 'text-sky-400 bg-sky-400/10',
...
function parseResultSlug(search: string): string | null {
...
export default function Quiz() {
  ...
  const sharedPreview = useMemo(() => getPersonalityBySlug(parseResultSlug(search)), [search]);
  const personality = useMemo(() => getPersonalityForScore(score, MAX_SCORE), [score]);
  const sharePayload = useMemo(
    () => buildQuizSharePayload(personality, score, MAX_SCORE),
    [personality, score]
  );
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-sm">
                    ...
                    Build streaks for bonus points — then unlock your shareable archetype.
                  </p>
                </div>
                ...
                {sharedPreview && (
                  <div
                    className={cn(
                      'rounded-xl border bg-white/5 px-4 py-3 text-left',
                      sharedPreview.accent === 'amber' ? 'border-accent/30' : 'border-primary/30'
                    )}
                  >
                    <p className="text-[11px] uppercase tracking-wider text-muted-foreground mb-1">A friend scored</p>
                    <p className="font-bold text-foreground">{sharedPreview.title}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">{sharedPreview.tagline}</p>
                  </div>
                )}
            ...
            {phase === 'finished' && (
              ...
              >
                <div className="text-center space-y-2">
                  <div className="w-14 h-14 rounded-full bg-gradient-to-br from-accent/30 to-primary/40 flex items-center justify-center mx-auto border border-accent/20 shadow-[0_0_40px_rgba(245,158,11,0.2)]">
                    <Trophy className="w-7 h-7 text-accent" />
                  </div>
                  <h2 className="text-2xl font-bold">Quiz Complete!</h2>
                  <p className="text-muted-foreground text-sm">Your demographic personality unlocked</p>
                </div>

                <div className="flex justify-center">
                  <ShareCard
                    title={personality.title}
                    subtitle={personality.tagline}
                    bigStat={`${scorePct}%`}
                    bigStatLabel={`${score} / ${MAX_SCORE} points`}
                    bullets={[personality.insight, personality.description]}
                    footer="populle.com/quiz"
                    accent={personality.accent}
                    className="min-h-[380px] aspect-auto sm:aspect-[9/16]"
                  />
                </div>
                ...
                <div className="glass-panel rounded-2xl p-4 space-y-4">
                    ...
                    <div className="flex flex-wrap justify-center gap-2">
                      {(['population', 'comparison', 'capital', 'flag', 'history'] as QuestionType[]).map((type) => {
                        ...
                        return (
                          <div
                            key={type}
                            className={cn('px-2 py-1 rounded-lg text-xs flex items-center gap-1', TYPE_COLORS[type])}
                          >
                  ...
                  <button
                    onClick={handleStart}
                    className="w-full py-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-foreground font-bold flex items-center justify-center gap-2 transition-all active:scale-95"
                  >
