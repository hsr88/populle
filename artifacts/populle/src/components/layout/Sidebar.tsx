...
import {
  Home, Sparkles, SplitSquareVertical, Globe2, Map, Building2,
  LineChart, PieChart, Brain, Info, HelpCircle, Shield, Github,
} from 'lucide-react';
...
const NAV_ITEMS = [
  ...
  { href: '/story', label: 'Your Story', icon: Sparkles },
  { href: '/contrast', label: 'Contrast', icon: SplitSquareVertical },
  ...
  return (
    <>
      ...
      <div className="hidden lg:flex w-64 h-screen fixed left-0 top-0 glass-panel border-r border-y-0 border-l-0 z-40 flex-col py-6">
        <Link href="/" className="flex items-center lg:px-6 mb-8 outline-none">
          <img
            ...
            return (
              <div key={item.href}>
                <Link href={item.href} className="outline-none">
                  <div
                    className={cn(
                      "relative flex items-center gap-4 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-300 group",
        ...
        <div className="mt-auto px-4 space-y-3">
          <div className="border-t border-white/8 pt-3">
            <p className="text-[11px] text-muted-foreground/80 leading-relaxed">
              ...
              Your story in the scale of humanity — from birth year to 2100.
            </p>
          </div>
          <div className="border-t border-white/8 pt-3">
            <a
      ...
      {/* Mobile top bar — primary viral items */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 glass-panel border-b border-white/10 px-3 py-3 flex items-center justify-between gap-2">
